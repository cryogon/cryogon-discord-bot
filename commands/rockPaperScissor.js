...
function updateScore(uWon, uLost) {
  ...
  winHistory[uWon.user.username] = {
    ...
    highest_streak: currHighestStreak
      ? currHighestStreak < currWinStreak + 1
        ? currWinStreak + 1
        : currHighestStreak
      : 1,
  };
  winHistory[uLost.user.username] = {
    win_streak: 0,
    highest_streak: score[uLost.user.username]?.highest_streak || 0,
  };
...
export const data = new SlashCommandBuilder()
  .setName("rps")
  .setDescription("Rock, Paper,& Scissor with another user")
  .addStringOption((option) => {
    return option
      .setName("action")
      .setDescription("Choose between rock,paper or scissor")
      .setChoices(
...
export async function execute(interaction) {
  ...
  collector.on("collect", async (i) => {
    if (i.user.id === interaction.user.id)
      i.reply({
        content: `You can't respond to your own request`,
        ephemeral: true,
    });
    else{
    ...
    await i.update({content:"Game is already complete",components:[]});
    await i.followUp(`${result}`)
